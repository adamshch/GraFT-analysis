...
else;             solveUse = 'quadprog'; % QUADPROG, TFOCS, FISTA, MPC
end

if iscell(D)
    if numel(D) == 1
        D   = D{1};
        DTD = double(2*(D.'*D));                                           % Quadratic objective term for lasso.
    elseif numel(D) == 2
        DTD = D{2};                                                        % Quadratic objective term for lasso.
        D   = D{1};
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Run the weighted LASSO to get the coefficients                                                                    % Optimization options
                                          
n = N2; %length(double(-2*D.'*mov_vec+lambda_val.*tau_vec));
if norm(mov_vec) == 0
    ...
    if nonneg
        switch lower(solveUse)
            ...
            case 'fista'
                par_spams.mode   = 2;
                par_spams.pos    = true;
                par_spams.lambda = 1; %max(mean(tau_vec),1e-3);
                S = mexLassoWeighted(double(mov_vec), D, ...
                          double(tau_vec), par_spams);   % ./par_spams.lambda
            otherwise
        end
    else
       opts.nonneg = false;
